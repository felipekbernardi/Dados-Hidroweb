...
for (k in 1:length(arquivos)){ # for criado para ler todos os arquivos da pasta 
  ...
  if(tipo_arquivo=="csv"){ # se for CSV abra csv
  ...
  for(t in 1:nrow(tabela_consistidos)){ # t e' o contador de linhas de consistidos
      ...
      if(teste_data==F){ # se ha valores, esses sao alocados na serie
        ...
        for(c in 14:(14+dias_no_mes)){ # a tabela comeca os dados na coluna 14 e termina no valor 14 + dias daquele mes
